...
//  console.log(val);
  if(val===undefined){
    invalidDatesList.push(new Date(date.year,String(date.month).padStart(2, '0'),String(i).padStart(2, '0'),10, 33, 30, 0));
  }
}
...
const monthNavigatorTemplate=(e)=> {
    return <Dropdown value={e.value} options={e.options} onChange={(event) => {onMonthChange(event.value);e.onChange(event.originalEvent, event.value)}}   className="p-ml-2" style={{ lineHeight: 1 }} />;
}
    ...
    <HomeLayout>
      ...
      <div>
        <div className="container" width="50%" >
                            ...
                            <div className="card">
                            <Calendar id="navigatorstemplate"  onSelect={(e) => onSelect(e.value)} monthNavigator yearNavigator yearRange="2021:2022" disabledDates={invalidDates}  showOtherMonths={false}
                                inline  dateTemplate={dateTemplate} monthNavigatorTemplate={monthNavigatorTemplate}  yearNavigatorTemplate={yearNavigatorTemplate} />
                                </div>
                        </div>
