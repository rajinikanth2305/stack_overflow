...
const UpComingTreks = ({ slice }) => {
  ...
  async function findDocs(pageNo,vpageSize,allTrue,filterOnlyFamilyTreks) {
   ...
   // page":1,"results_per_page":20,"results_size":2,"total_results_size":2,"total_pages":1,"next_page":null,"prev_page":null,"results"
    ...
    if(allTrue) {
      setCriteria({...criteria,all: true,familyTrek: false,season:'',difficulty:''});
      const doc = await client.query([Prismic.Predicates.at("document.type", "trek")],
     ...
     // console.log(JSON.stringify(response));
      ...
      setCriteria({...criteria,all: false,familyTrek: true,season:'',difficulty:''});
     ...
     // console.log(JSON.stringify(response));
      ...
      setCriteria({...criteria,all: false,familyTrek: false,season:{seasonValue},difficulty:{difficultyValue}});
    const doc = await client.query([
      Prismic.Predicates.at("document.type", "trek"),
      Prismic.Predicates.at("document.tags", [`${seasonValue}`,`${difficultyValue}`])],
      { pageSize: vpageSize, page: pageNo }
    ).then(function(response) {
    // response is the response object, response.results holds the documents
   // console.log(JSON.stringify(response));
              ...
              <div className="card ih_card shadow-md">
                <div className="card-body-padd">
                  <div className="d-flex align-items-center">
                    ...
                    <div className="mx-3">
                      <p className="m-0 link_text" onClick={clearAll}  >Clear all</p>
                    </div>
                  </div>
                  ...
                  <div className="row">
                    <div className="col-lg-6 col-md-12">
                      <div className="row">
                        <div className="col-lg-6 col-md-12">
                          <p className="m-0 link_text" onClick={findAll}>See All Treks</p>
                        </div>
                        <div className="col-lg-6 col-md-12">
                          <p className="m-0 link_text" onClick={findFamilyTreks}>Family Treks ?</p>
                        </div>
                      </div>
                    </div>
                    <div className="col-lg-6 col-md-12">
                      <div className="float-right">
                        <button className="btn btn-ih-green" onClick={find} >FIND TREKS</button>
                      </div>
                    </div>
                  </div>
                </div>
