...
const PostRender =  ({ data,authorData,updatesData,upComingData,relatedArticles }) => {
  ...
  const renderAuthorSlice=()=>{
    return(
    <div id="sidepanel_author_panel" className="col-lg-4 col-md-12">
    <div className="ml-100">
      <div className="text-center">
        <div className="auth_image">
          <img src={authorData?.data?.author_photo?.url} />
        </div>
        <p class="m-0 p-text-3-fg text-center mt-1">{authorData?.data?.author_first_name} {" "} {authorData?.data?.author_last_name}</p>
...
const renderRightImageLeftText =(slice)=> {
  ...
  <div className="col-lg-7 col-md-12">
    <div className="card">
      <div className="ar_img">
        <img   src={slice?.primary?.image?.url} width="100%" height="210px" />
      </div>
      <p className="p-text-small m-0 px-3 my-2">
      {slice?.primary?.image_caption}
      </p>
...
const renderUpComingTreks =() => {
  ...
  var slice= upComingData?.data?.body?.find(x=>x.slice_type==="best_treks_to_do");
...
const renderLatestVideos =() => {
  ...
  return(
    ...
    </p>
    {
        ...
        return(
    <div className="card border-bottom mb-3"  onClick={() => {
  ...
  return(
    ...
    <p className="p-text-3-fgc border-bottom-custom-1 pb-2">
                ...
                </p>
                 {
                  ...
                  let featureImageUrl="";
                  const featureSlice=article?.data?.body?.find(x=>x.slice_type=="feature_image");
                  if(featureSlice!=null) {
                     featureImageUrl = featureSlice.primary.feature_image.url;
                  }
                  const title=RichText.asText(article?.data.title);
                  const date=article?.data.date;
                  const author=article?.data?.author_first_name +  " " + article?.data?.author_last_name;
              
        return(
                <div className="border-bottom mb-3">
                  <div className="ar_right_side_imgs">
                    {featureImageUrl.length>0 && (
                  ...
                  {date} <span className="list-dot-style-mob"></span> 6
                    min read
                  </p>
                </div>
        );
              ...
              <div className="d-flex align-items-center">
                <div className="flex-grow-1">
                  <p className="m-0 p-text-small-black">
                    By <b>{data.author_first_name} {" "} {data.author_last_name}</b>
                  </p>
                  <p className="m-0 p-text-small-black">{data.date}</p>
                </div>
                <div>
                  <a href="">
                    <span className="social_bg mx-1">
