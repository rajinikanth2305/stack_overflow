...
const CancellationBp = () => {
  ...
  function fetchAndBindUserBatchBooking(email) {
    ...
    getBackPackOffloadingUserStatus(bookingId).then((bookingsData) => {
      if (bookingsData) {
  ...
  const setStates = (data) => {
    ...
    const bookingData = data?.filter(
      (x) => x.offloadingParticipantStatus !== "CANCELLED"
    );
  ...
  const buildParticipants = (userData, tcancelCharge) => {
    ...
    if (userData.offloadingAmountPaid !== userVoucherAppliedAmount) {
      if (
        userData.offloadingAmountPaid > 0 &&
        userData.cancellationPercentage > 0
      ) {
        cashRefund =
          ((100 - userData.cancellationPercentage) / 100) *
    ...
    if (userVoucherAppliedAmount > 0 && userData.cancellationPercentage > 0) {
      voucherPartAmount =
        ((100 - userData.cancellationPercentage) / 100) *
                    ...
                    <div className="col-lg-7 col-md-12">
                      <div className="d-flex align-items center p-cancel-text-fg">
                        ...
                        <div className="mx-3 col-8 m-l-border px-3">
                          <span>
                            Rs. {participants[0]?.trekFeeForTheUser} (incl.
                            taxes)
                          </span>
                          <p>{participants[0]?.trekName}</p>
                          <p>
                            {moment(participants[0]?.batchStartDate).format(
                              "DD MMM YYYY"
                            )}
                          </p>
                          <p>
                            {moment(participants[0]?.batchEndDate).format(
                              "DD MMM YYYY"
                            )}
                          </p>
                          {/* <p>
                      ...
                      <div className="card box-shadow">
                        <div className="p-3">
                          ...
                          <p className="p-text-3-fg mb-1">
                            ...
                            -
                            {moment(participants[0]?.batchEndtDate).format(
                              "DD MMM YYYY"
                            )}
                          </p>
