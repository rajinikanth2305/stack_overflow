...
import { getTrekLocations,saveUserLocations } from '../../../../services/queries';
...
const MyTreks =  forwardRef((props,ref) => {
 ...
 const onSubmit = (formData) => {
       ...
       if(userLocations.length>0) {
        ...
        saveUserLocations(participantData.bookingId,userLocations)
        .then(res=>{
          setSaveState(true);
          ...
          <tbody>
          {
                indexes?.map((index) => {
                 ...
                 // console.log(JSON.stringify(pdata));
                  ...
                  const name=pdata?.userDetailsForDisplay.email===participantData.email 
                              ? pdata?.userDetailsForDisplay.firstName +  pdata?.userDetailsForDisplay.lastName + ' (You) '
                              : pdata?.userDetailsForDisplay.firstName +  pdata?.userDetailsForDisplay.lastName;

                  const pickupLocations=locations.filter(x=>x.type==='PICKUP')
                  const dropLocations=locations.filter(x=>x.type==='DROP_OFF')

                  const currentPickupLocation=pdata?.pickupLocationId!==undefined 
                                   ? pdata?.pickupLocationId
                                   : null;
                  const currentDropLocation=pdata?.dropOffLocationId !==undefined 
