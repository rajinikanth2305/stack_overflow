...
import {
  getUserVoucher,getWooCustomerId,getWooCustomerOrders
...
const RentGear  = forwardRef((props, ref) => {
   ...
   const initData = (data) => {
    ...
    const filterTrek=(bookingTrekData,orders)=>{
      ...
      orders.map(order=>{
        ...
        const trek=order.meta_data.find(me=>me.key.toLowerCase()==="ih_order_trek_name");
  ...
  return (
    <>
          ...
          <div className="col-lg-6 col-md-12 col-12">
            ...
            {indexes?.map(index => {
              ...
              return(
                ...
                <div>
                  <p className="p-text-3-fgc mb-0">{index}.{item?.name}</p>
                </div>
                <div>
                  <p className="p-text-3-fgc mb-0">Rs. {item?.total}</p>
                </div>
              </div>
              )
             })
              ...
              <div className="d-flex justify-content-end align-items-center mb-2">
                ...
                <div>
                   {order.date_paid!=="" ? (
                    <p className="p-text-3-fgc mb-0">Rs. {order?.total}</p>
                   ): (
            ...
            <span className="btn table-btn-yellow">
            <a href="https://store.indiahikes.com/my-orders" target="new" >
               Modify/ Rent more
              </a>
              </span>
            </div>
      ...
      ):<p className="p-text-4 mb-0"><strong>Rent Gear information is not found for this Trek name and Date</strong></p>
      }
      </div>
