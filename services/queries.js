...
export const saveMyProfile = async (data) => {
  ...
  return axios.put(url, data, { headers: header }).then((res) => res.data);
};

const getTokenHeader = async () => {
  const obj = await auth.keycloak().then(([userTokenObject, userEmail]) => {
    return userTokenObject;
  });

  const token = await obj
    .updateToken()
    .then(function () {
      return obj.getToken();
    })
...
export const onAccept = async (email, batchId, ownerId) => {
  ...
  return axios.post(url, payload, { headers: header }).then((res) => res.data);
};
...
const buildTrekMates = (data, primaryUserEmail, stepName = "Default") => {
  ...
  data?.trekUsers?.map((y) => {
    ...
    if (stepName === "make_payment") {
      /// only make payment step saveDraft pass the voucher id if exists
      const userdata = {
        userId: y?.id,
...
export const cancelUserBooking = async (userEmail, bookingId) => {
  ...
  return axios.patch(url, {}, { headers: header }).then((res) => res.data);
};

export const cancelParticipantBooking = async (
  bookingId,
  moneyRefund,
  backPackOffloading,
  data
  ...
  return axios.patch(url, data, { headers: header }).then((res) => res.data);
};

export const cancelParticipantBackPack = async (
  bookingId,
  moneyRefund,
  backPackOffloading,
  data
...
export const saveUserLocations = async (bookingId, payload) => {
  ...
  console.log(JSON.stringify(payload));
  return axios.patch(url, payload, { headers: header }).then((res) => res.data);
};
...
export const uploadUserFitness = async (
  participantId,
  documentType,
  payload
) => {
...
export const saveUserReviews = async (reviewData) => {
  ...
  return axios
    .post(url, reviewData, { headers: header })
    .then((res) => res.data);
};

export const getUserFitnessDocuments = async (
  participantId,
  documentType,
  documentName
) => {
...
export const uploadUserIdProof = async (file, frontIdCard) => {
  ...
  const api = `${REACT_APP_TMS_BACKEND_URL}`;
  let url = frontIdCard
    ? `${api}/users/documents/ID_PROOF_FRONT`
    : `${api}/users/documents/ID_PROOF_BACK`;
  return axios.post(url, file, { headers: header }).then((res) => res.data);
};
